...
description: >
  ...
  未命中的新能力则新建条目。用户或 agent 改了文件后，凡是可能让上下文过时、
  或新增了尚未入库的能力，就必须使用。不限于显式调用：implement / rush 收尾要跑；
  用户直接对话改文件后，当前 agent 也要跑。避免 agent 不知道以代码还是文档为准。
---
...
4. 条目是否仍有效、是否应合并/废弃拿不准：用 <@交互式提问> 问用户，不要猜。
5. 多个条目逐个同步，不要串味。
...
汇报：扫描文件数、命中并更新的条目、新建的条目、`CODE-MAP.md` 是否更新、是否需要 `setup`。
