...
该单位 `goal.md` 为 `未确认`：停止，正在 explore，不要按旧需求写代码。
...
1. 运行 `node .agents/scripts/cooking.mjs status <feature>`，只看它的输出，不读各 `Pn.md` 正文。可做阶段以「可做」行为准。
2. 用户指定了 Pn：不在「可做」里则把它缺的前置（看输出里该阶段的「前置」列）告诉用户，停止。只给了 `P<n>` 未给标识：含该阶段的单位 0 个则停，1 个则用，多个则问。
