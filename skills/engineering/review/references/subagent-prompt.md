...
派发方把对应模板填好后，作为子代理的 **唯一** prompt。子代理没有父对话历史，只靠磁盘文件。不要把主对话里的实现过程、用户偏好或未写入磁盘的口头约定贴进 prompt。

`<engineering>` = `review/SKILL.md` 所在目录的上一级（`engineering/`）。
`<repo>` = 仓库根。
`<类别>` = 派发方前置检查得到的项目类别（代码 / 非代码）；写进任务书后子代理不再跑 precheck。

第一行必须是 `执行评审:`，用来标明执行方，防止再派一层。
...
完成后只汇报：结论（通过/不通过）、阻塞项原文。完整 git 评审写在你自己的对话里，不要写回父代理。
```
