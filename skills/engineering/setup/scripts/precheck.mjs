#!/usr/bin/env node
// setup 完成判定。检查项与 skills/engineering/setup/references/complete.md 一一对应，判定标准以该文件为准。
// 用法（以目标仓库根目录为工作目录）：
...
const ROOT_AGENTS_LINES = {
  代码: [
    ...
    '| `.agents/docs/CODE-MAP.md` | 定位模块；按模块/路径检索，禁止全文加载 | implement / sync-spec：模块表增删行，或某模块路径、入口、职责、依赖边变了。只改相关行。架构级变化先 setup 改 ARCHITECTURE，再由 setup 同步本文件。模块内部加文件不算。 |',
...
function main() {
  ...
  for (const file of ['.agents/docs/SPECS/index.md', '.agents/scripts/spec-files.mjs']) {
