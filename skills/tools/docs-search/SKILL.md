...
2. **中文用 2~6 字实体词**：「虚拟滚动」「分页」，不要写整句；「如何使用分页」里的「如何」「使用」会被当停用词剥掉，剩「分页」。
3. **组件名 + 场景词组合**：`"Table 分页"` 之类两三个词最有效；词太多 AND 条件太苛刻。
4. **空结果依次尝试**：减到 1~2 个核心词重搜 → 换同义词（`虚拟列表` ↔ `虚拟滚动`）→ 换英文标识符 → `libraries` 确认目标库存在 → `toc` 浏览该库 `index.md` 的结构找线索。
5. 命中后看 `snippet` 判断相关度，相关就 `get`；`sections` 字段已列出该文档全部章节名，按章节精确取可省 token。
